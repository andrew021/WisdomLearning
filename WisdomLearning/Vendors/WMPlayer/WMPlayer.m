...
#define kTopViewImageWidth  25
#define kTopViewImageHeight  25

const NSInteger kDownloadImageSize  = 2;
...
//        make.height.mas_equalTo(kTopViewImageHeight);
//        make.top.equalTo(self.topView).with.offset(5);
//        make.width.mas_equalTo(kTopViewImageWidth);
//        
//    }];
    ...
    [_collectButton setBackgroundImage:[UIImage imageNamed:WMPlayerSrcName(@"collect.png")] forState:UIControlStateNormal];
    [_collectButton addTarget:self action:@selector(buttonClicked:) forControlEvents:UIControlEventTouchUpInside];
//    _closeBtn.showsTouchWhenHighlighted = YES;
    [self.topView addSubview:_collectButton];
    ...
    [_shareButton setBackgroundImage:[UIImage imageNamed:WMPlayerSrcName(@"share.png")] forState:UIControlStateNormal];
...
//    _shareButton.showsTouchWhenHighlighted = YES;
    ...
    [self.shareButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.collectButton.mas_left).with.offset(-15);
        make.height.mas_equalTo(kTopViewImageHeight);
        make.top.equalTo(self.topView).with.offset(5);
        make.width.mas_equalTo(kTopViewImageWidth);
...
-(void)buttonClicked:(UIButton *)sender{
    if (sender == _collectButton) {
        ...
        NSLog(@"分享");
        if (_delegate && [_delegate respondsToSelector:@selector(wmplayer:clickShareButton:)]) {
            [_delegate wmplayer:self clickShareButton:sender];
        }
    }
}
#pragma mark
...
//        [self.layer insertSublayer:_playerLayer atIndex:0];
        ...
        if ([_playType isEqualToString:@"pptvideo"]){
            [self addSubview:_pptView];
            [self bringSubviewToFront:self.bottomView];
        }else{
            [self.layer insertSublayer:_playerLayer atIndex:0];
        }
...
-(void)fullScreenAction:(UIButton *)sender{
    ...
    if (sender.isSelected) {//全屏显示
        ...
        [self.hostViewController setNeedsStatusBarAppearanceUpdate];
...
//    self.currentItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:URLString]];
    ...
    self.pptView = [[UIImageView alloc] init];
    self.pptView.frame = self.layer.bounds;
...
- (void)moviePlayDidEnd:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self.bottomView.alpha = 1.0;
        self.topView.alpha = 1.0;
        [self.pptView sd_setImageWithURL:[NSURL URLWithString:_pptImgList[0]]];
    } completion:^(BOOL finish){
...
//    [self.player seekToTime:CMTimeMakeWithSeconds(slider.value, _currentItem.currentTime.timescale) toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero completionHandler:^(BOOL finished) {
//        
//    }];
//    
...
//    // add-xucg
//    if (_isTry && slider.value > _tryTime) {
//        [self.progressSlider setValue:_tryTime animated:YES];
...
- (void)actionTapGesture:(UITapGestureRecognizer *)sender {
    ...
    [self.player seekToTime:CMTimeMakeWithSeconds(self.progressSlider.value, self.currentItem.currentTime.timescale)];
//    [self.player seekToTime:CMTimeMakeWithSeconds(self.progressSlider.value, _currentItem.currentTime.timescale) toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero completionHandler:^(BOOL finished) {
//        
//    }];
//    if (self.player.rate != 1.f) {
//        if ([self currentTime] == [self duration])
...
- (void)syncScrubber{
    ...
    if (isfinite(duration)){
        ...
        [self setPPTViewImageByTime:nowTime];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (verValue > horValue) {//上下滑动
        ...
        // add-xucg
        if (_isTry && self.progressSlider.value > _tryTime){
            [self.progressSlider setValue:_tryTime animated:YES];
            [self.player seekToTime:CMTimeMakeWithSeconds(self.progressSlider.value, self.currentItem.currentTime.timescale)];
...
- (void)toFullScreenWithInterfaceOrientation:(UIInterfaceOrientation )interfaceOrientation
{
    ...
    self.pptView.frame = CGRectMake(0,0, kScreenHeight,kScreenWidth);
    ...
    if (IOS_VERSION < 8.0 ) {
        self.loadFailedLabel.center = CGPointMake(kScreenWidth/2-36, -(kScreenWidth/2)+36);
//        [self.loadFailedLabel mas_updateConstraints:^(MASConstraintMaker *make) {
////            make.width.mas_equalTo(kScreenHeight);
...
-(void)playToTime:(double)time{
    [self.player seekToTime:CMTimeMakeWithSeconds(time, self.currentItem.currentTime.timescale)];
}

-(void)setPPTViewImageByTime:(double) time{
    if (time == 0) {
        return ;
    }
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSInteger index = [self getPptImageIndexByTime:time];
        
        for (NSInteger k = index+1, num=0; k < _pptImgList.count&&num<=kDownloadImageSize; ++k) {
            NSString *nowString = _pptImgList[k];
            [self advancedDownloadImageByUrl:nowString];
            num++;
        }
        
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *urlString = _pptImgList[index];
            if ([_curPptimageString isEqualToString:urlString] == NO) {
                [_pptView sd_setImageWithURL:[NSURL URLWithString:urlString]];
                _curPptimageString = urlString;
            }
...
-(void)setPptImgList:(NSArray *)pptImgList{
    _pptImgList = pptImgList;
    NSString *urlString = pptImgList[0];
//    [self advancedDownloadImageByUrl:urlString];
    [_pptView sd_setImageWithURL:[NSURL URLWithString:urlString] placeholderImage:[UIImage imageNamed:@"no_picture"]];
    
    for (NSInteger k = 1, num=0; k < _pptImgList.count&&num<=kDownloadImageSize; ++k) {
        NSString *nowString = _pptImgList[k];
        [self advancedDownloadImageByUrl:nowString];
        num++;
    }
...
-(void)advancedDownloadImageByUrl:(NSString *)urlString{
    NSURL *url = [NSURL URLWithString:urlString];
    SDWebImageManager *manager = [SDWebImageManager sharedManager];
    
    if ([manager cachedImageExistsForURL:url] || [manager diskImageExistsForURL:url]) {
        return ;
    }else{
...
-(NSInteger)getPptImageIndexByTime:(double)time
{
    if (time <= [_pptVideoIndex[0] doubleValue]) {
        return 0;
    }
    if (time >= [[_pptVideoIndex lastObject] doubleValue]) {
        return _pptVideoIndex.count-1;
    }
    
    NSInteger low = 0;
    NSInteger high = _pptVideoIndex.count-1;
    
    while (low <= high) {
        NSInteger mid = (low + high)/2;
        if (time >= [_pptVideoIndex[mid] doubleValue]  && time <= [_pptVideoIndex[mid+1] doubleValue] ) {
            return mid+1;
        }
        
        if (time > [_pptVideoIndex[mid] doubleValue]) {
            low = mid+1;
        }
