...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
		...
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat imageYOffset  = (CGRectGetHeight(self.scrollView.frame)-(imageHeight+stringHeight+5))/2;
...
//            CGFloat textYOffset = roundf((CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2.0f);
            
            CGRect imageRect = CGRectMake(imageXOffset, imageYOffset, imageWidth, imageHeight);
            CGFloat textYOffset = CGRectGetMaxY(imageRect) + 5;
            CGRect textRect = CGRectMake(textXOffset, textYOffset, textWidth, stringHeight);
