...
- (void)viewDidLoad {
   ...
   // UIView * view;
    ...
    if(_isAdd){
         ...
         CGRect  frame1 = CGRectMake(0, 50, self.view.frame.size.width, self.view.frame.size.height);
         self.sourceView = [[ZSSTextView alloc] initWithFrame:frame1];
         self.editorView = [[UIWebView alloc] initWithFrame:frame1];

        
    }
    else{
       self.shouldShowKeyboard = NO;
       // CGRect frame2 = CGRectMake(0, 10, self.view.frame.size.width, self.view.frame.size.height-10);
   
         CGRect frame3 = CGRectMake(0, _height, self.view.frame.size.width, self.view.frame.size.height-_height);
        self.sourceView = [[ZSSTextView alloc] initWithFrame:frame3];
        self.editorView = [[UIWebView alloc] initWithFrame:frame3];
    }
...
//    [self.view addSubview:self.sourceView];
   ...
   // self.editorView = [[UIWebView alloc] initWithFrame:frame];
    ...
    self.toolbarHolder.backgroundColor = [UIColor colorWithRed:254/255.0f green:244/255.0f blue:217/255.0f alpha:1];
    //[UIColor yellowColor];
...
//        UIButton * insertImage = [UIButton buttonWithType:UIButtonTypeCustom];
//        [insertImage setImage:[UIImage imageNamed:@"richText_camera"] forState:UIControlStateNormal];
...
//        [items addObject:insertImage];
        UIImage *leftImage = [[UIImage imageNamed:@"richText_camera"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
       

        ZSSBarButtonItem *insertImage = [[ZSSBarButtonItem alloc] initWithImage:leftImage style:UIBarButtonItemStyleDone target:self action:@selector(insertImage)];
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarInsertLink]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
         ...
         UIImage *leftImage = [[UIImage imageNamed:@"richText_photo"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        ZSSBarButtonItem *insertLink = [[ZSSBarButtonItem alloc] initWithImage:leftImage style:UIBarButtonItemStylePlain target:self action:@selector(insertLink)];
...
- (void)buildToolbar {
    ...
    NSLog(@"++++%@ %lu",items,items.count);
    UIBarButtonItem *space = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace
                                                                           target:nil
                                                                           action:nil];
    
    [arr insertObject:space atIndex:1];
  ...
  //  self.toolbar.backgroundColor = [UIColor yellowColor];
    ...
    //self.toolBarScroll.backgroundColor = [UIColor yellowColor];
}
...
//    if(_delegate){
//        [self.delegate selectImagesInsertWithTag:2];
//    }
    // Save the selection location
//    [self.editorView stringByEvaluatingJavaScriptFromString:@"zss_editor.prepareInsert();"];
//    
...
- (void)insertImage {
    ...
    // Save the selection location
//    if(_delegate){
//        [self.delegate selectImagesInsertWithTag:1];
//    }
//    [self.editorView stringByEvaluatingJavaScriptFromString:@"zss_editor.prepareInsert();"];
//    
...
//                //修改 代理
//                if(_delegate){
//                    [self.delegate selectImagesInsert];
//                }
