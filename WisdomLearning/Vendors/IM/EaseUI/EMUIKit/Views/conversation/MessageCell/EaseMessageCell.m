    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.bodyType) {
            case EMMessageBodyTypeText:
            {
                ...
                _bubbleView.isText = YES;
                // 2016.6.12
//                NSString *OBJECT_NAME = self.model.message.ext[@"OBJECT_NAME"];
//                if (!OBJECT_NAME) {
//                    self.bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.text textFont:[UIFont systemFontOfSize:14]];;
//                }else{
//                    NSMutableAttributedString *aAtt = [[NSMutableAttributedString alloc]initWithString:model.text];
//                    NSUInteger length = OBJECT_NAME.length;
//                    [aAtt addAttribute:NSForegroundColorAttributeName value:[UIColor colorFromHexString:@"0e60f7"] range:NSMakeRange(12, length)];
//                    [aAtt addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:14] range:NSMakeRange(0, model.text.length)];
//                    self.bubbleView.textLabel.attributedText = aAtt;
//                }
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.imageView sd_setImageWithURL:[_model.fileURLPath stringToUrl] placeholderImage:[UIImage imageNamed:_model.failImageName]];
                    } else {
...
//                    }
                    self.bubbleView.voiceImageView.image = self.model.isSender ? [UIImage imageNamed:@"send_voice_3"] : [UIImage imageNamed:@"receive_voice_3"];
                    _bubbleView.voiceImageView.animationImages = self.model.isSender ? self.sendMessageVoiceAnimationImages:self.recvMessageVoiceAnimationImages;
                }
            ...
            case EMMessageBodyTypeVideo:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.videoImageView sd_setImageWithURL:[_model.fileURLPath stringToUrl] placeholderImage:[UIImage imageNamed:_model.failImageName]];
                    } else {
