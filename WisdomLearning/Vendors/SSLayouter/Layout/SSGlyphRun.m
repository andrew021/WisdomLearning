...
@implementation SSGlyphRun
{
    ...
    __weak SSLayoutLine *_line;	// retain cycle, since these objects are retained by the _line
...
- (void)drawDecorationInContext:(CGContextRef)context
{
    ...
    UIColor *backgroundColor = [self.attributes backgroundColor];
...
- (NSArray *)stringIndices
{
    if (!_stringIndices)
    {
        ...
        //        if (!indices)
        //        {
        //            CFRange runRange = CTRunGetStringRange(_run);
        //            CFIndex *buf = malloc(sizeof(CFIndex*) * runRange.length );
        //            memset(buf, '\0', runRange.length);
        //            CTRunGetStringIndices(_run, runRange, buf);
        //            indices = buf;
        //        }
...
- (void) setStringRange:(NSRange)range;
{
    _stringRange = range;
}
