...
//    highlightedImage = [UIImage imageNamed:@"app_item_bg"];
   ...
   // deleteIconImage = [UIImage imageNamed:@""];
    ...
    for (NSInteger index = 0; index < self.showGridTitleArray.count; index++) {
        ...
        if ([gridTitle isEqualToString:@"添加更多"]) {
            isAddDelete = NO;
        }
    //    NSLog(@"%@",CGRectZero);
...
//    [self updateNewFrame];
    ...
    if(_gridViewDelegate){
        [self.gridViewDelegate deletateButtonTag:deleteButton.tag];
    }
...
- (void)sortGridList
{
    ...
    NSLog(@"你好  %lu",(unsigned long)_gridListArray.count);
    //更新所有格子的中心点坐标信息
    for (NSInteger i = 0; i < _gridListArray.count; i++) {
...
-(void)updateNewFrame
{
    ...
    if (self.gridListArray.count % PerRowGridCount== 0) {
        ...
        //  NSLog(@"图片个数 %lu",(unsigned long)self.gridListArray.count);
    }
    else{
       // NSLog(@"图片个数 %lu",(unsigned long)self.gridListArray.count);
