...
@interface FilterView ()<GetClickBtnText,UITextFieldDelegate>
...
- (instancetype) initWithFrame:(CGRect)frame withNSArray:(NSArray*)arr
{
    if (self = [super initWithFrame:frame]){
        ...
        _filter0 = model1.filterTypeCode;
        _filter1 = model.filterTypeCode;
        ...
        for(int i=0;i<2;i++){
            ...
            btn.frame = CGRectMake(15+SCREEN_WIDTH/4*i, ViewMaxY(self.tagList)+10, SCREEN_WIDTH/4-15, 30);
...
-(void)clickSelectBtn:(UIButton*)btn{
    ...
    if(btn.tag==0){
        ...
        NSLog(@"%@ %@",self.downString,self.highString);
        if(_delegate){
          ...
          //  [self.delegate sendDownPrice:self.downString withHighPrice:self.highString withCodeArr:self.selectCodeArr];
            
            [self.delegate clickSureInFilterView:self];
           //  NSLog(@"%@ %@ %@",self.downString,self.highString,self.selectCodeArr);
        }
    }
}
@end
