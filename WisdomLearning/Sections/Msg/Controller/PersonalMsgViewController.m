...
@interface PersonalMsgViewController ()<DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
                                    ...
                                    modelForConversation:(EMConversation*)conversation
{
    ...
    if (zmodel.conversation.type == EMConversationTypeChat) {
        ZSMessageFriendListModel* contact = [[ContactsManager sharedContactsManager] contactModelByUserId:conversation.conversationId];
        if (contact) {
            zmodel.title = contact.USER_SHORTNAME;
    
            if(contact.USER_PIC.length>0){
            
                zmodel.avatarURLPath =[NSString stringWithFormat:@"%@%@",gIconHost,contact.USER_PIC];

            }
            else{
                zmodel.avatarImage = KPlaceHeaderImage;
            }
        }
        else {
            ZSIndividualDetail* zsModel = [[ContactsManager sharedContactsManager] getLinkManModel:conversation.conversationId];
            
          
            NSString* nickName = zsModel.userNick;
            NSString* userAvatar = zsModel.userAvater;
              NSLog(@"+++++ %@",zsModel.userNick);
            zmodel.title = nickName;
            if(zsModel.userAvater.length>0){
                zmodel.avatarURLPath =[GetImageUrl getImageUrl:userAvatar];
            }else{
                zmodel.avatarImage = KPlaceHeaderImage;
      ...
      latestMessageTimeForConversationModel:(id<IConversationModel>)conversationModel
{
    ...
    ;
    if (lastMessage) {
        latestMessageTime =   [NSDate formatLastMessageTime:lastMessage.timestamp]; //[NSDate formattedTimeFromTimeInterval:lastMessage.timestamp];
      
        //latestMessageTime = @"2016-10-14";
    }
