...
@interface SearchCerViewController ()<UITableViewDataSource, UITableViewDelegate,DZNEmptyDataSetDelegate,DZNEmptyDataSetSource>
{
...
@implementation SearchCerViewController
...
- (void)createRefresh
{
    __weak SearchCerViewController* weakSelf = self;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   // CertificateDetailViewController *vc = [[CertificateDetailViewController alloc]init];
    CertificateListModel * model = _dataArray[indexPath.row];
   // vc.cerId = model.certId;
   // [self.navigationController pushViewController:vc animated:YES];
    
    
    if(_delegate){
        [self.delegate interactData:nil tag:4 data:model.certId];
    }
}
